...
this be a field of context, and add MAYBE_STATIC throughout (probably
...
Plan:
  ...
  * move global_options to be a field of context::
    ...
    In a GLOBAL_STATE this is effectively::
      ...
      context::global_options_.x_name_of_option
  ...
  * then a series of patches to remove the places where the macros are
    ...
    the lookup above explicit.  Where a more specific context can be
...
frontends and backends.  Hence context could contain::
...
users of context.h to include tree.h
...
Perhaps add methods to context for looking up trees, and use these?

Alternatively, introduce a wrapper class for this: `struct global_trees_d`
and have a singleton within the context.
...
and make dump_file be a field of context, rather than a global (as in the
`class context` above.  This would be a largish patch though: adding a lookup
...
identify that the context isn't used, and optimize away the lookups
as equivalent to::
...
within the context, with const_int_rtx to become a macro::
...
Plan: move the array into context, though perhaps a new frontend class
...
Plan: move into context, perhaps within a new class recog (using
MAYBE_STATIC)::
...
of the cfun approach: make a field inside context::
...
TODO: not yet sure how to tackle this; perhaps another field in context
hidden with a macro?
...
Idea: if we put it in context, put a `context *` into each
...
Plan: TODO: perhaps convert to a field of context, add a compatibility
macro to get it relative to a local `context *`, and see how much still
...
Plan:
  ...
  * introduce a `class cp_state` (or `class cp_frontend`) to hold
    ...
    does it need a `context *`?  If we can get away with just providing
